...
namespace Joomla\Plugin\Sociallogin\Twitch\Extension;
...
use Joomla\Plugin\Sociallogin\Twitch\Integration\OAuth as TwitchOAuth;
use Joomla\Plugin\Sociallogin\Twitch\Integration\UserQuery;
    ...
    public function init(): void {
      ...
      protected function getConnector(): TwitchOAuth {
          if (is_null($this->connector)) {
            $options = ['clientid' => $this->appId, 'clientsecret' => $this->appSecret, 'redirecturi' => Uri::base() . 'index.php?option=com_ajax&group=sociallogin&plugin=' . $this->integrationName . '&format=raw', 'requestparams' => ['claims' => '{"id_token":{"email":null,"email_verified":null},"userinfo":{"email":null,"email_verified":null,"picture":null,"preferred_username":null,"updated_at":null}}', 'state' => $this->getApplication()->getSession()->getToken()]];
            $httpClient = HttpFactory::getHttp();
            $this->connector = new TwitchOAuth($options, $httpClient, $this->getApplication()->input, $this->getApplication());
